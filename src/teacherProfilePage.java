...
public class teacherProfilePage extends JFrame {
    ...
    public teacherProfilePage(String enrollmentNumber) {
        ...
        editButton.setBounds(360, 550, 200, 50);
        ...
        backButton.setBounds(30, 550, 200, 50);
        ...
        firstNameLabel = createLabel("FIRST NAME:", 20, startY, 400);
        lastNameLabel = createLabel("LAST NAME:", 280, startY, 400);
        emailLabel = createLabel("EMAIL:", 20, startY + spacing +10, 450);
        phoneNumberLabel = createLabel("PHONE NUMBER:", 20, startY + 2 * spacing +30, 450);
        enrollmentLabel = createLabel("ENROLLMENT NUMBER:", 20, startY + 3 * spacing + 50, 450);
        passwordLabel = createLabel("PASSWORD:", 20, startY + 4 * spacing + 70, 450);
        bloodGroupLabel = createLabel("BLOOD GROUP:", 20, startY + 5 * spacing + 90, 400);
        genderLabel = createLabel("GENDER:", 280, startY + 5 * spacing + 90, 400);
        cityLabel = createLabel("CITY:", 20, startY + 6 * spacing + 110, 400);
        stateLabel = createLabel("STATE:", 280, startY + 6 * spacing + 110, 400);
    ...
    private void updateDatabase(String columnName, String newValue, String fieldName) {
        try (Connection connection = DatabaseConnection.getConnection();
             PreparedStatement preparedStatement = connection.prepareStatement(
                     "UPDATE teachers_info SET " + columnName + " = ? WHERE enrollment_number = ?")) {
    ...
    private void fetchUserDataAndDisplay() {
        try (Connection connection = DatabaseConnection.getConnection();
             PreparedStatement preparedStatement = connection.prepareStatement("SELECT * FROM teachers_info WHERE enrollment_number = ?")) {
    ...
    public static void main(String[] args) {
        new teacherProfilePage("S24CSEUXXXX");
    }
}